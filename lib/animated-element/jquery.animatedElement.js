$(document).ready(function() {
    ...
    $('.animated-children').bind('inview', function() {
        ...
        if ( $this.hasClass('from-left') )   { setTimeout(function() { $children.animate({opacity: 1, left: 0},   {duration: duration}); }, delay); }
        if ( $this.hasClass('from-right') )  { setTimeout(function() { $children.animate({opacity: 1, right: 0},  {duration: duration}); }, delay); }
        if ( $this.hasClass('from-top') )    { setTimeout(function() { $children.animate({opacity: 1, top: 0},    {duration: duration}); }, delay); }
        if ( $this.hasClass('from-bottom') ) { setTimeout(function() { $children.animate({opacity: 1, bottom: 0}, {duration: duration}); }, delay); }
        if ( $this.hasClass('appear') )      { setTimeout(function() { $children.animate({opacity: 1},            {duration: duration}); }, delay); }
        if ( $this.hasClass('lift') )        { setTimeout(function() { $children.animate({opacity: 1, bottom: 0}, {duration: duration}); }, delay); }
        if ( $this.hasClass('drop') )        { setTimeout(function() { $children.animate({opacity: 1, top: 0},    {duration: duration}); }, delay); }
    });
