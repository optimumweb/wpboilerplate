$(document).ready(function() {
    ...
    $('.animate-element').bind('inview', function() {
        ...
        if ( $this.hasClass('lift') ) { $this.animate({bottom: 0}, duration); $this.fadeTo(duration/4, 1); }
        if ( $this.hasClass('drop') ) { $this.animate({top: 0}, duration); $this.fadeTo(duration/4, 1); }
    });
