...
class Validation {
                ...
                if ( isset( $field['required'] ) ) {
                    switch ( $field['required'] ) {
                        ...
                        case 'tel':
                            ...
                            break;
                        case 'shield':
                            $is_valid = $this->is_valid_shield( $field['value'] );
                            break;
                        default:
    ...
    public function validate_length( $min_len = null, $max_len = null, $value = null ) {
    ...
    public function is_longer_than( $len, $value ) {
        return $this->is_valid_string( $value ) && strlen( $value ) > $len;
    }
    ...
    public function is_between( $min_len, $max_len, $value ) {
        return $this->is_valid_string( $value ) && $this->is_longer_than( $min_len, $value ) && $this->is_shorter_than( $max_len, $value );
    }

    public function is_alphanum( $value ) {
        return $this->is_valid_string( $value ) && ctype_alnum( $value );
    }

    public function is_valid_email( $value ) {
        return $this->is_valid_string( $value ) && filter_var( $value, FILTER_VALIDATE_EMAIL );
    }

    public function is_valid_tel( $value ) {
        $value = preg_replace( "/[^0-9]/", "", $value );
        return $this->is_valid_string( $value ) && $this->is_between( 6, 15, $value );
    }

    public function is_valid_shield( $value ) {
        return isset( $value ) && is_string( $value ) && strlen( $value ) === 0;
    }
}
